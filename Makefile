# Anime Aggressors — repo-native targets

.PHONY: vxp2-structural vxp2-capture vxp2-gates engineering-wave011 engineering-wave012 engineering-wave013b engineering-wave014 engineering-wave015 engineering-wave016 engineering-wave017 engineering-wave018 engineering-wave019 engineering-wave020 engineering-wave021 engineering-wave022 wave020-fighter-select-diagnostic wave020-visibility-diagnostic wave020-select-flourish-diagnostic wave020-pause-diagnostic wave020-audio-diagnostic wave020-battle-body-diagnostic wave020-pause-layout-diagnostic wave020-victory-diagnostic wave020-select-lifecycle-diagnostic wave020-material-persistence-diagnostic wave020-transform-isolation-diagnostic wave020-pixel-cp2-seal wave021-art-direction wave021-form-architecture wave021-aura-tiers wave021-ember-ascension wave021-battle-scale wave021-ui-feel motion-promote wave015-human-crash-capture wave015-battlescene-stability taste-gate engineering-taste-gate visual-golden-qa art-pipeline-infra combat-161-v3

engineering-wave011:
	bash tools/engineering_wave011/run_wave011.sh

engineering-wave012:
	bash tools/engineering_wave012/run_wave012.sh

engineering-wave013b:
	bash tools/engineering_wave013b/run_wave013b.sh

engineering-wave014:
	bash tools/engineering_wave014/run_wave014.sh

engineering-wave015:
	bash tools/engineering_wave015/run_wave015.sh

engineering-wave016:
	bash tools/engineering_wave016/run_wave016.sh

engineering-wave017:
	bash tools/engineering_wave017/run_wave017.sh

engineering-wave018:
	bash tools/engineering_wave018/run_wave018.sh

engineering-wave019:
	bash tools/engineering_wave019/run_wave019.sh

engineering-wave020:
	bash tools/engineering_wave020/run_wave020.sh

engineering-wave021:
	bash tools/engineering_wave021/run_wave021.sh

engineering-wave022:
	bash tools/engineering_wave022/run_wave022.sh

combat-161-v3:
	bash tools/combat_v3/run_v3.sh

wave022-form-architecture:
	bash tools/engineering_wave022/run_diagnostic.sh form-architecture

wave022-battle-scale:
	bash tools/engineering_wave022/run_diagnostic.sh battle-scale

wave022-ui-feel:
	bash tools/engineering_wave022/run_diagnostic.sh ui-feel

wave022-full-roster-ascension:
	bash tools/engineering_wave022/run_diagnostic.sh full-roster-ascension

wave022-rook-ascension:
	bash tools/engineering_wave022/run_diagnostic.sh rook-ascension

wave021-art-direction:
	python3 tools/engineering_wave021/check_art_direction.py

wave021-form-architecture:
	bash tools/engineering_wave021/run_diagnostic.sh form-architecture

wave021-aura-tiers:
	bash tools/engineering_wave021/run_diagnostic.sh aura-tiers

wave021-ember-ascension:
	bash tools/engineering_wave021/run_diagnostic.sh ember-ascension

wave021-battle-scale:
	bash tools/engineering_wave021/run_diagnostic.sh battle-scale

wave021-ui-feel:
	bash tools/engineering_wave021/run_diagnostic.sh ui-feel

wave020-fighter-select-diagnostic:
	bash tools/engineering_wave020/run_fighter_select_diagnostic.sh

wave020-visibility-diagnostic:
	bash tools/engineering_wave020/run_visibility_diagnostic.sh

wave020-select-flourish-diagnostic:
	bash tools/engineering_wave020/run_select_flourish_diagnostic.sh

wave020-pause-diagnostic:
	bash tools/engineering_wave020/run_pause_diagnostic.sh

wave020-battle-body-diagnostic:
	bash tools/engineering_wave020/run_battle_body_diagnostic.sh

wave020-pause-layout-diagnostic:
	bash tools/engineering_wave020/run_pause_layout_diagnostic.sh

wave020-victory-diagnostic:
	bash tools/engineering_wave020/run_victory_diagnostic.sh

wave020-select-lifecycle-diagnostic:
	bash tools/engineering_wave020/run_select_lifecycle_diagnostic.sh

wave020-material-persistence-diagnostic:
	bash tools/engineering_wave020/run_material_persistence_diagnostic.sh

wave020-transform-isolation-diagnostic:
	bash tools/engineering_wave020/run_transform_isolation_diagnostic.sh

wave020-pixel-cp2-seal:
	python3 tools/engineering_wave020/run_pixel_cp2_physical_seal.py

wave020-audio-diagnostic:
	bash tools/engineering_wave020/run_audio_diagnostic.sh

wave015-human-crash-capture:
	python3 tools/engineering_wave015/capture_human_play_crash.py

wave015-battlescene-stability:
	python3 tools/engineering_wave015/run_battlescene_stability.py

motion-promote:
	bash tools/motion_pipeline/production/promote_contribution.sh

# Game Taste Gate — placeholder detect + model visibility static + report emit.
# Does not invent Pixel evidence or assign HUMAN_Q5.
taste-gate engineering-taste-gate:
	python3 tools/quality/check_placeholder_visuals.py
	python3 tools/quality/check_model_visibility_reliability.py
	python3 tools/quality/emit_taste_gate_report.py

# Golden Visual QA v2 — extends v1 with Wave021 ideal states (non-invasive).
# Exits non-zero on material deviations. Never marks OWNER_APPROVED_GOLDEN.
visual-golden-qa:
	python3 tools/quality/run_visual_golden_qa.py

.PHONY: stream-c-exhaust
stream-c-exhaust:
	python3 tools/digital_engineering_exhaustion/stream_c/run_stream_c.py


# VXP-2 — brand / presentation foundation (fixture evidence only; never invents Pixel/human PASS)
vxp2-structural:
	bash tools/vxp2/run_structural.sh

vxp2-capture:
	bash tools/vxp2/run_capture.sh

vxp2-gates:
	python3 tools/vxp2/emit_gates.py

art-pipeline-infra:
	python3 tools/art_pipeline/human_art/emit_infra_gates.py
	python3 tools/art_pipeline/human_art/tests/test_infra.py
	python3 tools/authored_animation/validate_provenance.py
