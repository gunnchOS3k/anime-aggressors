...
function prepDamage(s = createInitialDerbyState(9)) {
  s.phase = "damage";
  s.phaseFrame = 0;
  s.timerFramesRemaining = DAMAGE_PHASE_FRAMES;
  s.player.x = 1190 * 256;
  s.dummy.x = 1200 * 256;
  return s;
}
...
describe("Impact Dummy Derby", () => {
  ...
  it("final launch distance increases with damage", () => {
    ...
    low.phase = "finalLaunch";
    low.phaseFrame = 0;
    low.kineticBat.equipped = true;
    low.kineticBat.available = true;
    ...
    high.phase = "finalLaunch";
    high.phaseFrame = 0;
    high.kineticBat.equipped = true;
    high.kineticBat.available = true;
  ...
  it("kinetic bat final hit launches farther than normal attack only", () => {
    ...
    assert.ok(bat.finalDistance > distAfterNormal);
  });
  ...
  it("retry resets state correctly", () => {
    ...
    assert.equal(retry.personalBest, 500);
  });
});
