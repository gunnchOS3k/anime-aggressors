...
export function startLaneBlaster(canvas: HTMLCanvasElement) {
  ...
  let bullets: Array<{ x: number, y: number, vx: number, vy: number, owner: number }> = [];
  let enemies: Array<{ x: number, y: number, lane: number, vx: number }> = [];
  ...
  trackGamepads((pads, dt) => {
    ...
    if (p1.axes[0] > 0.5 && players[0].lane < lanes - 1) players[0].lane++;
    
    // Player 2 movement
    if (p2.axes[0] < -0.5 && players[1].lane > 0) players[1].lane--;
    if (p2.axes[0] > 0.5 && players[1].lane < lanes - 1) players[1].lane++;
