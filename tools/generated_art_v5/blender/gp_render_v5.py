"""Render generated-art v5 review packet with v4 contract-forward cameras."""
...
from generated_art_v5.render_review import PACKET  # noqa: E402
        ...
        for mat in bpy.data.materials:
            ...
            for node in mat.node_tree.nodes:
                if node.type in {"EMISSION"}:
                    cache.setdefault(mat.name, []).append(
                        (node, [c for c in node.inputs["Color"].default_value], node.inputs["Strength"].default_value if "Strength" in node.inputs else 0.0)
                    )
