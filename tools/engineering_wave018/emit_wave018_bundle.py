...
def emit_result() -> dict:
    ...
    pixel_avail = bool(pixel_sel.get("PIXEL_DEVICE_AVAILABLE", False)) or pixel_sel.get("PIXEL_CAMPAIGN") == "PASS"
    ready = ghosts_ok and uplift and desktop_ok

    status = "PASS" if ready else "PARTIAL"
    if pixel_sel.get("PIXEL_CAMPAIGN") == "BLOCKED_PIXEL6A" and ready:
        status = "PASS"  # Pixel optional when desktop proofs pass; report blocked honestly
    ...
    payload = {
        ...
        "VISIBLE_BODY_DUPLICATE_OCCURRENCES": battle.get("VISIBLE_BODY_DUPLICATE_OCCURRENCES", select.get("VISIBLE_BODY_DUPLICATE_OCCURRENCES")),
