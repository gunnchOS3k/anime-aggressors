...
from generated_art_v9.attachment_map import CLASSES, classify_name
from generated_art_v9.body_profiles import (
    FIGHTER_IDS,
    STYLE_LOCK_V9,
    STYLE_PROFILES,
    load_pairs,
...
from generated_art_v9.hero_poses import idle_v9, overlay_table, pose_delta_ok, pose_v9
from generated_art_v9.impact_pair_solver import geometry_ok, solve_from_rest
from generated_art_v9.pair_scenes import defender_for
from generated_art_v9.render_review import REQUIRED_SHOTS
...
class V8IntegrityTests(unittest.TestCase):
    def test_doc_exists(self):
        self.assertTrue(STYLE_LOCK_V9.is_file())
        text = STYLE_LOCK_V9.read_text(encoding="utf-8")
    ...
    def test_poses_cover_acting(self):
        for fid in FIGHTER_IDS:
            idle = idle_v9(fid)
            self.assertTrue(pose_delta_ok(idle, pose_v9(fid, "hurt_peak"), ("Head", "Chest", "Hips", "UpperArm_R", "UpperLeg_R")), fid)
            self.assertTrue(pose_delta_ok(idle, pose_v9(fid, "charge"), ("Chest", "Head", "Hips", "UpperArm_R", "UpperArm_L")), fid)
            self.assertTrue(pose_delta_ok(idle, pose_v9(fid, "super"), ("UpperArm_R", "UpperArm_L", "Chest", "Hips")), fid)
    ...
    def test_idle_identities_differ(self):
        distinct = 0
        ember = idle_v9("ember-vale")
        for fid in FIGHTER_IDS:
            if fid == "ember-vale":
                continue
            if pose_delta_ok(ember, idle_v9(fid), ("Hips", "Chest", "Head", "UpperArm_R", "UpperArm_L", "UpperLeg_R")):
                distinct += 1
    ...
    def test_packet_includes_contact_and_stress(self):
        ...
        self.assertIn("heavy_contact_off", REQUIRED_SHOTS)
        self.assertIn("glove_fist_silhouette", REQUIRED_SHOTS)
        self.assertIn("boot_silhouette", REQUIRED_SHOTS)
        self.assertIn("charge_100_silhouette", REQUIRED_SHOTS)

    def test_hero_proportions_are_not_universal(self):
        from generated_art_v9.body_profiles import load_hero_profiles, style_profile

        heroes = load_hero_profiles()
        shoulders = {round(float(heroes[fid]["shoulder_scale"]), 3) for fid in FIGHTER_IDS}
        hands = {round(float(heroes[fid]["hand_scale"]), 3) for fid in FIGHTER_IDS}
        self.assertGreaterEqual(len(shoulders), 5)
        self.assertGreaterEqual(len(hands), 5)
        rook = style_profile("rook-ironside")
        juno = style_profile("juno-spark")
        self.assertGreater(rook.shoulder_width, juno.shoulder_width)
        self.assertGreater(rook.hand_scale, juno.hand_scale)
        self.assertGreater(rook.foot_scale, juno.foot_scale)
    ...
    def test_roadmap_mentions_v9(self):
...
def json_load_human_check() -> dict:
    ...
    path = Path(__file__).resolve().parents[3] / "game-godot/data/art/generated_v9/palette_blocking.json"
