"""Build one v9 fighter: heroic proportions, classified costume, armature-owned bind."""
...
from generated_art_v5.rig_bind import bind_to_bone  # noqa: E402
...
from generated_art_v9.attachment_map import tag_object  # noqa: E402
from generated_art_v9.body_profiles import as_v5_body, hero_feature_scale, load_hero_profiles, style_profile  # noqa: E402
from generated_art_v9.boot_builder import build_boots  # noqa: E402
from generated_art_v9.cel_materials import apply_review_color_management, assign, build_materials, paint_undersuit  # noqa: E402
from generated_art_v9.costume_masses import build_costume  # noqa: E402
from generated_art_v9.export_runtime import export_glb, report_payload  # noqa: E402
from generated_art_v9.glove_builder import build_cast_gloves, build_gloves, build_guard_gloves, build_open_gloves  # noqa: E402
from generated_art_v9.hero_poses import overlay_table  # noqa: E402
from generated_art_v9.rig_bind_v8 import bind_character_v8  # noqa: E402
...
def build_v9_fighter(fid: str, p, arm):
    ...
    open_hands = build_open_gloves(bp, arm)
    guard_hands = build_guard_gloves(bp, arm)
    cast_hands = build_cast_gloves(bp, arm)
    for hand in open_hands + guard_hands + cast_hands:
        assign(hand, mats["glove"])
        hand.hide_render = True
        hand.hide_viewport = True
        hand["aa_review_variant"] = 1
    boots = build_boots(bp, arm)
    for boot in boots:
        assign(boot, mats["boot"])
    costume = build_costume(bp, arm, mats)
    extras = [head] + hands + open_hands + guard_hands + cast_hands + boots + costume
    outlines = []
    for src in extras + [body]:
        hull = add_outline_hull(src, mats["outline"], width=0.016)
        if hull is not None:
            hull.parent = src
            outlines.append(hull)
    meshes = [body] + extras
    gap = snap_to_ground(meshes + outlines)
    bind_info = bind_character_v8(body, extras, arm)
    paint_undersuit(body, mats)
    ...
    if any(mod.type == "REMESH" for obj in meshes for mod in obj.modifiers):
        raise RuntimeError("v9 must not use remesh modifiers")
    tris = sum(len(obj.data.polygons) for obj in meshes)
    hero = load_hero_profiles()[fid]
    return body, extras, {
        "construction": "profile_loft_no_voxel_remesh",
        "visible_style": "graphic_lowpoly_cel_combat",
        "hero_proportions": hero,
        "hero_feature_scale": hero_feature_scale(fid),
        ...
        "hero_feature_parts": sum(1 for obj in costume if obj.get("aa_hero_feature")),
        "undersuit": True,
        "glove_thumb": True,
        "boot_sole": True,
        ...
        "attachments": bind_info["attachments"],
        "unintentional_floating": bind_info["unintentional_floating"],
        ...
        "hero_feature": bp.hero_feature,
    }


def _overlay_v9_hero_poses(fid: str, arm) -> None:
    overlays = overlay_table(fid)
...
def build(fid: str, out_blend: Path, out_glb: Path, skip_anim: bool) -> dict:
    ...
    body, costume, geom = build_v9_fighter(fid, p, arm)
    _link(mesh_col, body)
    for obj in costume:
        _link(mesh_col, obj)
    ...
    if ground is None:
        ...
        _assign(ground, _toon_mat_v3("AA_Ground", (0.08, 0.08, 0.09), 0.0, 2, 0.04, 0.0, 0.92))
    actions = [] if skip_anim else apply_animations(fid, arm)
    if not skip_anim:
        _overlay_v9_hero_poses(fid, arm)
