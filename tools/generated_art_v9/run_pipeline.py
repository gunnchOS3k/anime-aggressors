#!/usr/bin/env python3
"""Run generated art v9 rigged-costume + contact pipeline. Does not merge. Does not set HUMAN_*."""
...
from generated_art_v9.emit_gates import main as emit_gates  # noqa: E402
from generated_art_v9.export_runtime import write_glb_import  # noqa: E402
from generated_art_v9.make_roster_sheets import main as make_sheets  # noqa: E402
from generated_art_v9.validate_attachment_integrity import validate as validate_attach  # noqa: E402
from generated_art_v9.validate_geometry import validate as validate_geom  # noqa: E402
from generated_art_v9.validate_visual_metrics import inspect as inspect_stills  # noqa: E402
from generated_art_v9.write_az_report import main as write_az  # noqa: E402
...
def generate_masters() -> dict:
    ...
    script = ROOT / "tools/generated_art_v9/blender/gp_build_v9.py"
    reports = {}
...
def render_packet(blender: str) -> dict:
    render = ROOT / "tools/generated_art_v9/blender/gp_render_v9.py"
    v8_dir = ROOT / "artifacts/vxp3/review/generated_art_v9"
    rows = {}
    for fid in FIGHTER_IDS:
        blend = production_master_blend(fid).resolve()
...
def _write_camera_rollup() -> dict:
    ...
    write_json(REPORTS / "REVIEW_CAMERA_ORIENTATION_V9.json", out)
    return out
...
def main() -> int:
    ...
    write_json(REPORTS / "GENERATED_ART_V9_MASTERS.json", masters)
    if masters.get("ok") and masters.get("blender"):
        renders = render_packet(masters["blender"])
        write_json(REPORTS / "GENERATED_ART_V9_RENDERS.json", renders)
    _write_camera_rollup()
    try:
        sheets = make_sheets()
    except Exception as exc:  # noqa: BLE001
        sheets = {"ok": False, "reason": str(exc)}
    write_json(REPORTS / "GENERATED_ART_V9_SHEETS.json", sheets)
    geometry = validate_geom()
    try:
        inspect_stills()
    except Exception as exc:  # noqa: BLE001
        write_json(REPORTS / "GENERATED_ART_V9_QUALITY.json", {"ok": False, "reason": str(exc)})
    try:
        validate_attach()
    except Exception as exc:  # noqa: BLE001
        write_json(REPORTS / "GENERATED_ART_V9_ATTACHMENT_INTEGRITY.json", {"ok": False, "reason": str(exc)})
    emit_gates()
    write_az()
