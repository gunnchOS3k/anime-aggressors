#!/usr/bin/env python3
"""Run generated art v6 graphic style-lock pipeline. Does not merge. Does not set HUMAN_*."""
...
from generated_art_v6.body_profiles import FIGHTER_IDS  # noqa: E402
from generated_art_v6.emit_gates import main as emit_gates  # noqa: E402
from generated_art_v6.export_runtime import write_glb_import  # noqa: E402
from generated_art_v6.make_roster_sheets import main as make_sheets  # noqa: E402
from generated_art_v6.validate_geometry import validate as validate_geom  # noqa: E402
from generated_art_v6.validate_visual_metrics import inspect as inspect_stills  # noqa: E402
from generated_art_v6.write_az_report import main as write_az  # noqa: E402
...
def generate_masters() -> dict:
    ...
    script = ROOT / "tools/generated_art_v6/blender/gp_build_v6.py"
    render = ROOT / "tools/generated_art_v6/blender/gp_render_v6.py"
    reports = {}
    v6_dir = ROOT / "artifacts/vxp3/review/generated_art_v6"
...
def _write_camera_rollup() -> dict:
    ...
    write_json(REPORTS / "REVIEW_CAMERA_ORIENTATION_V6.json", out)
    return out
...
def main() -> int:
    ...
    write_json(REPORTS / "GENERATED_ART_V6_MASTERS.json", masters)
    _write_camera_rollup()
    try:
        sheets = make_sheets()
    except Exception as exc:  # noqa: BLE001
        sheets = {"ok": False, "reason": str(exc)}
    write_json(REPORTS / "GENERATED_ART_V6_SHEETS.json", sheets)
    geometry = validate_geom()
    try:
        inspect_stills()
    except Exception as exc:  # noqa: BLE001
        write_json(REPORTS / "GENERATED_ART_V6_QUALITY.json", {"ok": False, "reason": str(exc)})
    emit_gates()
    write_az()
