"""v6 geometry/construction gates. File existence is not a visual pass."""
...
from generated_art_v6.render_review import REQUIRED_SHOTS, ROSTER_SHEETS
...
def validate() -> dict:
    ...
    for fid in FIGHTER_IDS:
        ...
        gloves = gloves and hand_parts >= 5 and bool(geom.get("glove_thumb"))
        boots = boots and boot_parts >= 5 and bool(geom.get("boot_sole"))
        masks = masks and head_parts >= 3
        masses = masses and costume_parts >= 4
        suit = suit and bool(geom.get("undersuit"))
        tris_ok = tris_ok and 5000 <= tris <= 35000
        fighters[fid] = {
    ...
    payload = {
        "ok": loft and gloves and boots and masks and masses and suit and not remesh,
        "GEN_ART_V6_NO_VOXEL_REMESH": loft and not remesh,
        "GEN_ART_V6_STYLE_LOCK_DIGITAL": style and loft,
        "GEN_ART_V6_FULL_SUIT_COVERAGE_DIGITAL": suit,
        "GEN_ART_V6_GLOVE_READ_DIGITAL": gloves,
        "GEN_ART_V6_BOOT_READ_DIGITAL": boots,
        "GEN_ART_V6_MASK_HEAD_READ_DIGITAL": masks,
        "GEN_ART_V6_COSTUME_MASS_DIGITAL": masses,
        "triangle_budget_ok": tris_ok,
        "roster_sheets": sheets,
        "fighters": fighters,
    }
    REPORTS.mkdir(parents=True, exist_ok=True)
    (REPORTS / "GENERATED_ART_V6_GEOMETRY.json").write_text(json.dumps(payload, indent=2) + "\n")
    return payload
