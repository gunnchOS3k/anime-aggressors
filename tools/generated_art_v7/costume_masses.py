"""v7 costume hero features. Leave dark undersuit visible so value groups can split."""
...
from generated_art_v7.cel_materials import assign
...
def build_costume(profile: StyleProfile, arm_obj, mats: dict):
    ...
    if fid == "ember-vale":
        ...
        add(_shell("torso_shell", stations, scale=1.10, start=1, end=4), "armor")
        add(_guard("sharp_calf_r", ll_h, ll_t, 0.040), "accent")
        add(_guard("sharp_calf_l", ll_hl, ll_tl, 0.040), "accent")
        add(box_mesh("volt_sash", mid + Vector((0.04, 0.09, 0.01)), (0.36, 0.07, 0.12)), "emit", hero=True)
        add(box_mesh("volt_panel_a", mid + Vector((0.10, 0.10, 0.03)), (0.16, 0.09, 0.20)), "accent", hero=True)
        add(box_mesh("volt_panel_b", mid + Vector((-0.06, 0.06, -0.02)), (0.10, 0.05, 0.12)), "accent")
    elif fid == "kaia-windrow":
        add(_shell("torso_shell", stations, scale=1.12, start=1, end=3), "armor")
        _nh, nt = bone_pts(arm_obj, "Neck")
        sl_h, sl_t = bone_pts(arm_obj, "Shoulder_L")
        sr_h, sr_t = bone_pts(arm_obj, "Shoulder_R")
        add(box_mesh("scarf_collar", nt + Vector((0.0, 0.04, 0.01)), (0.18, 0.12, 0.09)), "accent", hero=True)
        add(box_mesh("scarf_fall", nt + Vector((0.06, 0.08, -0.14)), (0.08, 0.12, 0.28)), "accent", hero=True)
        add(box_mesh("airfoil_l", sl_t + Vector((0.05, 0.03, 0.08)), (0.10, 0.05, 0.20)), "accent")
        add(box_mesh("airfoil_r", sr_t + Vector((-0.05, 0.03, 0.08)), (0.10, 0.05, 0.20)), "accent")
        add(_guard("thigh_mass_l", ul_hl, ul_tl, 0.050 * profile.thigh_mass), "secondary")
    elif fid == "nix-calder":
        # Dark suit stays visible on arms/waist/neck. Pale armor is chest, shoulders, forearms, shins.
        add(box_mesh("frost_cuirass", mid + Vector((0.0, 0.11, 0.02)), (0.32, 0.14, 0.24)), "armor", hero=True)
        add(box_mesh("frost_plastron", mid + Vector((0.0, 0.14, -0.02)), (0.20, 0.10, 0.16)), "armor", hero=True)
        add(box_mesh("frost_pauldron_r", ua_h + Vector((-0.04, 0.04, 0.04)), (0.16, 0.12, 0.12)), "armor", hero=True)
        add(box_mesh("frost_pauldron_l", ua_hl + Vector((0.04, 0.04, 0.04)), (0.16, 0.12, 0.12)), "armor")
        ...
        add(_shell("coat_layer", stations, scale=1.26, start=1, end=7), "secondary")
        add(_shell("torso_shell", stations, scale=1.14, start=1, end=4), "armor")
        add(box_mesh("authority_panel", mid + Vector((0.0, 0.12, 0.01)), (0.20, 0.08, 0.18)), "accent", hero=True)
        add(box_mesh("orbit_trim", mid + Vector((0.0, 0.04, 0.12)), (0.28, 0.22, 0.05)), "emit", hero=True)
        add(box_mesh("orbit_ring", Vector((0.08, 0.14, chest_t.z + 0.12)), (0.34, 0.26, 0.04)), "emit")
        ...
        add(_shell("torso_shell", stations, scale=1.12, start=1, end=3), "armor")
        add(box_mesh("coat_panel_l", hips_h + Vector((0.16, 0.06, 0.12)), (0.18, 0.14, 0.32)), "secondary", hero=True)
        add(box_mesh("coat_tail_l", hips_h + Vector((0.18, 0.04, -0.12)), (0.09, 0.18, 0.36)), "secondary", hero=True)
        add(box_mesh("coat_tail_r", hips_h + Vector((-0.08, 0.02, -0.10)), (0.06, 0.12, 0.26)), "secondary")
        add(box_mesh("void_trim", mid + Vector((0.02, 0.11, 0.01)), (0.16, 0.06, 0.14)), "accent")
