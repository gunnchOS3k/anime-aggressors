"""v7 cel materials. Standard view transform so value groups survive review."""
from __future__ import annotations

import bpy

from generated_art_v7.body_profiles import load_palettes
...
def cel_mat(name, color, emit=0.02, bands=3, rim=0.16, slot="ARMOR", shadow=None):
    ...
    shade = shadow or (color[0] * 0.42, color[1] * 0.38, color[2] * 0.44)
    mid = (color[0] * 0.88, color[1] * 0.86, color[2] * 0.90)
    hi = (min(1.0, color[0] * 1.08), min(1.0, color[1] * 1.06), min(1.0, color[2] * 1.04))
    for i, (pos, val) in enumerate(zip([0.20, 0.52, 0.80][:bands], [(*shade, 1.0), (*mid, 1.0), (*hi, 1.0)])):
...
def build_materials(fid: str) -> dict:
    ...
    return {
        ...
        "secondary": cel_mat(f"{fid}.mat.SECONDARY", pal["accent"] if fid == "vesper-nyx" else pal["glove"], 0.02, 3, 0.12, "SECONDARY"),
