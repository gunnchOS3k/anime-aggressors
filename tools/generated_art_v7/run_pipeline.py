#!/usr/bin/env python3
"""Run generated art v7 graphic hero polish pipeline. Does not merge. Does not set HUMAN_*."""
...
from generated_art_v7.emit_gates import main as emit_gates  # noqa: E402
from generated_art_v7.export_runtime import write_glb_import  # noqa: E402
from generated_art_v7.make_roster_sheets import main as make_sheets  # noqa: E402
from generated_art_v7.validate_geometry import validate as validate_geom  # noqa: E402
from generated_art_v7.validate_visual_metrics import inspect as inspect_stills  # noqa: E402
from generated_art_v7.write_az_report import main as write_az  # noqa: E402
...
def generate_masters() -> dict:
    ...
    script = ROOT / "tools/generated_art_v7/blender/gp_build_v7.py"
    reports = {}
...
def render_packet(blender: str) -> dict:
    render = ROOT / "tools/generated_art_v7/blender/gp_render_v7.py"
    v7_dir = ROOT / "artifacts/vxp3/review/generated_art_v7"
    rows = {}
    for fid in FIGHTER_IDS:
        blend = production_master_blend(fid).resolve()
...
def _write_camera_rollup() -> dict:
    ...
    write_json(REPORTS / "REVIEW_CAMERA_ORIENTATION_V7.json", out)
    return out
...
def main() -> int:
    ...
    write_json(REPORTS / "GENERATED_ART_V7_MASTERS.json", masters)
    if masters.get("ok") and masters.get("blender"):
        renders = render_packet(masters["blender"])
        write_json(REPORTS / "GENERATED_ART_V7_RENDERS.json", renders)
    _write_camera_rollup()
    try:
        sheets = make_sheets()
    except Exception as exc:  # noqa: BLE001
        sheets = {"ok": False, "reason": str(exc)}
    write_json(REPORTS / "GENERATED_ART_V7_SHEETS.json", sheets)
    geometry = validate_geom()
    try:
        inspect_stills()
    except Exception as exc:  # noqa: BLE001
        write_json(REPORTS / "GENERATED_ART_V7_QUALITY.json", {"ok": False, "reason": str(exc)})
    emit_gates()
    write_az()
