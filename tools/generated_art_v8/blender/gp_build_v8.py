"""Build one v8 fighter: lofted body, classified costume, armature-owned bind."""
...
from generated_art_v8.attachment_map import tag_object  # noqa: E402
from generated_art_v8.body_profiles import as_v5_body, style_profile  # noqa: E402
from generated_art_v8.boot_builder import build_boots  # noqa: E402
from generated_art_v8.cel_materials import apply_review_color_management, assign, build_materials, paint_undersuit  # noqa: E402
from generated_art_v8.costume_masses import build_costume  # noqa: E402
from generated_art_v8.export_runtime import export_glb, report_payload  # noqa: E402
from generated_art_v8.glove_builder import build_gloves, build_guard_gloves, build_open_gloves  # noqa: E402
from generated_art_v8.hero_poses import overlay_table  # noqa: E402
from generated_art_v8.rig_bind_v8 import bind_character_v8  # noqa: E402
...
def build_v8_fighter(fid: str, p, arm):
    ...
    for hand in open_hands + guard_hands:
        assign(hand, mats["glove"])
    ...
    extras = [head] + hands + open_hands + guard_hands + boots + costume
    outlines = []
    for src in extras + [body]:
        hull = add_outline_hull(src, mats["outline"], width=0.011)
        if hull is not None:
            hull.parent = src
    ...
    if any(mod.type == "REMESH" for obj in meshes for mod in obj.modifiers):
        raise RuntimeError("v8 must not use remesh modifiers")
...
def _overlay_v8_hero_poses(fid: str, arm) -> None:
...
def build(fid: str, out_blend: Path, out_glb: Path, skip_anim: bool) -> dict:
    ...
    body, costume, geom = build_v8_fighter(fid, p, arm)
    _link(mesh_col, body)
    for obj in costume:
        _link(mesh_col, obj)
    ...
    if not skip_anim:
        _overlay_v8_hero_poses(fid, arm)
