"""Render generated-art v8 review packet with contact solver and attachment stress."""
...
from generated_art_v8.cameras import frame_pair, frame_subject, mesh_bounds  # noqa: E402
from generated_art_v8.cel_materials import apply_review_color_management, assign, build_materials, mute_emission  # noqa: E402
from generated_art_v8.hero_poses import pose_v8  # noqa: E402
from generated_art_v8.impact_pair_solver import contact_spec, solve_from_armatures  # noqa: E402
from generated_art_v8.pair_scenes import defender_for, pair_offsets  # noqa: E402
from generated_art_v8.render_review import PACKET, STRESS_SHOTS  # noqa: E402
...
def _ensure_defender(fid: str) -> list[str]:
    ...
    if blend.is_file():
        ...
        for obj in data_to.objects:
            ...
            if obj.type == "MESH" and (obj.name.lower().endswith("_open") or obj.name.lower().endswith("_guard")):
                obj.hide_render = True
                obj.hide_viewport = True
...
def _stage_pair(fid: str, attacker, label: str) -> tuple[list[str], dict]:
    ...
    if def_arm is not None:
        ...
        if label == "clash_push":
            hurt = "clash_push"
        if label == "heavy_anticipation_pair":
            hurt = "hurt_pre"
        if label == "heavy_follow":
            hurt = "launch"
        _apply_pose(def_arm, pose_v8(defender_id, hurt))
        spec = contact_spec(fid)
        contact = solve_from_armatures(attacker, def_arm, spec["socket"], spec["anchor"], tuple(spec.get("normal") or (1.0, 0.0, 0.0)))
...
def main() -> int:
    ...
    for label, action, frame, preset_name, sil, gray, outline, aim, pair, vfx, emission_off in PACKET:
        ...
        if pair:
            ...
            loc, target, lens, ortho = frame_pair(contact=contact)
        elif aim in {"glove", "glove_open", "glove_guard", "boot", "head", "costume"}:
