...
def run_anime(game: dict[str, str]) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if (ROOT / "package.json").exists():
        ...
        cmds["validate_audio_closure"] = run_cmd(["npm", "run", "validate:anime-digital-art-audio-closure"], timeout=300)
        dims["audio"] = dim(
            "PASS" if cmds["validate_audio_closure"]["ok"] else "FAIL",
            "npm run validate:anime-digital-art-audio-closure",
            notes="Subjective mix quality remains HUMAN_VALIDATION_REQUIRED",
        )
        ...
        cmds["test_netplay"] = run_cmd(["npm", "run", "test:netplay"], timeout=600)
        cmds["test_rollback"] = run_cmd(["npm", "run", "test:rollback"], timeout=600)
        net_ok = cmds["test_netplay"]["ok"] and cmds["test_rollback"]["ok"]
        dims["networking"] = dim(
            "PASS" if net_ok else ("PARTIAL" if cmds["test_netplay"]["ok"] or cmds["test_rollback"]["ok"] else "FAIL"),
            "npm run test:netplay + test:rollback",
            None if net_ok else "HUMAN_VALIDATION_REQUIRED",
        )
