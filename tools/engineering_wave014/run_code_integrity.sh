...
cd "$ROOT"
mkdir -p artifacts/engineering_wave014
python3 - <<'PY'
import json, re
from pathlib import Path
root = Path(".")
prod_roots = [Path("game-godot/scripts"), Path("game-godot/scenes")]
hardcoded = re.compile(r"ENGINEERING_WAVE_014\s*=\s*[\"']PASS[\"']")
...
Path("artifacts/engineering_wave014/CODE_INTEGRITY_RESULT.json").write_text(json.dumps(out, indent=2) + "\n")
