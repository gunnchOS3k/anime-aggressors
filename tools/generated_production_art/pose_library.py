...
def poses_for_action(fid: str, action: str) -> dict[str, Pose]:
    from .hero_poses_v4 import (  # noqa: WPS433 — avoid import cycle at module load
        charge_100_v4,
        idle_v4,
        super_sequence_v4,
    )
    from .hero_poses_v3 import (  # noqa: WPS433
        heavy_sequence_v3,
    ...
    if action in {"aura_burst_super_pose", "signature_lane_finisher", "super"}:
        return super_sequence_v4(fid)
    if action in {"charged_idle", "charge_full"}:
        charged = charge_100_v4(fid)
        return {
            "SETTLE": idle,
