...
func set_hit_flash(intensity: float = 1.0) -> void:
	...
	for mesh in _mesh_instances:
		...
		mesh.modulate = base.lerp(Color(1.4, 0.5, 0.5), clampf(intensity, 0.0, 1.0))
...
func _collect_meshes(node: Node) -> void:
	if node is MeshInstance3D:
		...
		_base_colors[node.get_instance_id()] = node.modulate
...
func _apply_team_tint() -> void:
	for mesh in _mesh_instances:
		if mesh == null:
			continue
		var base: Color = _base_colors.get(mesh.get_instance_id(), Color.WHITE)
		mesh.modulate = base.lerp(team_color, 0.18)
