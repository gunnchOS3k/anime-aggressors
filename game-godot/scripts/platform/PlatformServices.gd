...
func save_path(relative: String) -> String:
	## Desktop/Steam policy: user://. Console paths stay abstract until SDK access.
...
func bridge_achievement(achievement_id: String) -> void:
	## Offline catalog remains source of truth. Steam/console bridges are stubs.
...
func entitlements_ready() -> bool:
	## Always true offline. Console entitlement checks remain EXTERNAL_PENDING.
	return true
