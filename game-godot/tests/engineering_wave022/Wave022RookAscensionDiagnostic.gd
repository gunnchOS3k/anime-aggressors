...
## Wave022 Rook micro-gate — >=30 transform activations (§33 batch order fighter #2).

const BATTLE_PATH := "res://scenes/battle/BattleScene.tscn"
const OUT_PATHS := [
	"res://../artifacts/engineering_wave022/ROOK_ASCENSION_RESULT.json",
	"../artifacts/engineering_wave022/ROOK_ASCENSION_RESULT.json",
]

const TARGET_ACTIVATIONS := 30
const FIGHTER_ID := "rook-ironside"
const BASE_FORM := "ROOK_BASE"
...
func _run() -> void:
	...
	if fighter == null:
		_write({"ok": false, "reason": "rook fighter missing"})
		quit(1)
		return
	...
	for _i in range(TARGET_ACTIVATIONS):
		...
		if after > before:
			...
			failures += 1
		if str(fighter.get_current_form_id()).is_empty():
			form_mismatches += 1
		var model = fighter.model_3d
		if model != null and model.has_method("get_body_scale_contract"):
			if float(model.get_body_scale_contract().get("body_scale", 1.0)) > 1.05:
				scale_violations += 1

	var ok := activations >= TARGET_ACTIVATIONS and failures == 0 and scale_violations == 0
	var payload := {
		"ok": ok,
		...
		"FAILURES": failures,
		"ROOK_ASCENSION_DESKTOP": "PASS" if ok else "FAIL",
		"emitted_at": Time.get_datetime_string_from_system(true),
	}
	_write(payload)
	print("ROOK_ASCENSION ok=", ok, " activations=", activations)
	quit(0 if ok else 1)
