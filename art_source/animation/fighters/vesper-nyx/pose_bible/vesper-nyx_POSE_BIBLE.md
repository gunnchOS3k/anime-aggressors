# Pose Bible — Vesper Nyx

**Status:** TEMPLATE ONLY. Not human-authored. `AUTHORED_POSE_BIBLE_PASS=false`.

- Fighter: `vesper-nyx`
- Lane: void / mix
- Identity: Asymmetric idle. Feint then confirm. Dissolve-snap hurt.
...
1. Open `source/vesper-nyx_control_rig.blend` (when LFS source is present).
2. Pose deform or FK controls. Screenshot orthographic front + side.
3. Drop stills in this folder as `vesper-nyx_<slot>_front.png`.
