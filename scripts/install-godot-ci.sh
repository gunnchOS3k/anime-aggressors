#!/usr/bin/env bash
# Install Godot editor binary for CI or local macOS validation.
...
INSTALL_DIR="${RUNNER_TOOL_CACHE:-/tmp}/godot/${GODOT_VERSION}"

case "$(uname -s)" in
  Darwin)
    GODOT_BIN="${INSTALL_DIR}/Godot.app/Contents/MacOS/Godot"
    ZIP_NAME="Godot_v${GODOT_VERSION}-stable_macos.universal.zip"
    ;;
  Linux)
...
ZIP_URL="https://github.com/godotengine/godot/releases/download/${GODOT_VERSION}-stable/${ZIP_NAME}"
...
if [ -x "${GODOT_BIN}" ]; then
  ...
  TMP="$(mktemp -d)"
  trap 'rm -rf "${TMP}"' EXIT
  echo "Downloading Godot ${GODOT_VERSION} for $(uname -s)..." >&2
