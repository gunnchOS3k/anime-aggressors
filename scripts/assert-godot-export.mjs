...
import { validateGodotPagesExportRoot } from "./godot-export-shared.mjs";

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const root = process.env.GODOT_EXPORT_ROOT ?? path.join(repoRoot, "apps/web/dist/godot");

const result = validateGodotPagesExportRoot(root, { label: root });
...
console.log("Godot Pages export OK:", root);
console.log("  boot shell:", path.join(root, "index.html"));
console.log("  rescue runtime:", path.join(root, "rescue-runtime.js"));
console.log("  runtime dir:", path.join(root, "runtime"));
